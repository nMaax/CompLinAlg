...
%% Implementation Inverse Power Method and deflation

function [s_eigval, s_eigvec] = InversePowerMethod(A, p, max_iter, rel_tol)
...
function Px = householder_mat(x)
    ...
    Px = eye(length(u)) - 2 * (u * u');

    return;
end
function [processed] = DeflationProcess(A, x)
    [m,n] = size(A);
    if n~=m
        error('Not square matrix')
    end
